...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!invoiceUrl) {
      console.error("❌ Missing InvoiceLink. Full response:", JSON.stringify(invoiceRes.data, null, 2));
    ...
    if (err.response?.data) {
      console.error("💥 PayPro Error:", JSON.stringify(err.response.data, null, 2));
    }

    return res.status(500).json({
      error: 'Order creation failed',
      message: err.response?.status === 406
        ? 'PayPro rejected the invoice request (HTTP 406). Check payload or merchant ID.'
        : err.message
    });
  }
}
