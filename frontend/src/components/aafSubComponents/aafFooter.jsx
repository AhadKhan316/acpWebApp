...
const AafFooter = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: "error", content: "An error occurorange. Please try again." });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const quickLinks2 = [
    { id: 1, text: "Arts Alumni Festival", href: "/festival/aaf" },
    ...
    { id: 7, text: "Women Conference", href: "/festival/wc" },
  ];
          ...
          <motion.div custom={22} variants={childVariants}>
            ...
            >
              ...
              <motion.button
                type="submit"
                disabled={isLoading}
                className={`w-full bg-orange-800 text-white font-semibold py-2 rounded transition ${isLoading ? "opacity-50 cursor-not-allowed" : "hover:bg-orange-700"}`}
              ...
              {message.content && (
                <p className={`mt-2 text-sm ${message.type === "success" ? "text-green-500" : "text-orange-500"}`}>
                  {message.content}
                </p>
              )}
            </motion.form>
